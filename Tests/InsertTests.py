...
class ImportTests(Records,General):
    ...
    def test_create_incrementing_id(self):
        ...
        self.assertLess(json.loads(first_response.text)["metadata"]["createdRecordIds"],json.loads(second_response.text)["metadata"]["createdRecordIds"])
    @pytest.mark.Insert
    ...
    def test_all_fields_correct_data(self):
        response = self.Insert(table="brfnk4id5", id=['6','7','8','9','10','11'], values=['Text',2,0.2,self.generate_now_time(),"some@mail.com","https://quickbase.com"], fields_to_return="")
        self.assertEqual(response.status_code , 200)
    @pytest.mark.Insert
